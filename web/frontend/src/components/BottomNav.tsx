...
export function BottomNav({ tab, onChange, language }: Props) {
  const signLabel = !language || language.code === 'MALAY'
    ? 'Isyarat → Suara'
  ...
  const avatarLabel = !language || language.code === 'MALAY'
    ? 'Teks → Avatar'
    : language.code === 'MANDARIN'
    ? '文本 → 虚拟人'
    : language.code === 'TAMIL'
  ...
  return (
    <nav className="nav" role="tablist">
      ...
      >
        ...
        <span className="nav__label">{signLabel}</span>
      </button>
      ...
      >
        ...
        <span className="nav__label">{avatarLabel}</span>
      </button>
    </nav>
  )
}
