...
/**
 * Covers [GonkaTranslator.extractTranslation] against the shapes models
...
class TranslationParsingTest {
    ...
    fun `parses a clean JSON object`() {
        val t = GonkaTranslator.extractTranslation(
    ...
    fun `rejects a missing language`() {
        assertThrows(IllegalArgumentException::class.java) {
            GonkaTranslator.extractTranslation("""{"ms": "Saya lapar.", "en": "I am hungry."}""")
        }
    }
    ...
    fun `rejects a reply missing Tamil`() {
        assertThrows(IllegalArgumentException::class.java) {
            GonkaTranslator.extractTranslation(
    ...
    fun `rejects a blank language`() {
        assertThrows(IllegalArgumentException::class.java) {
            GonkaTranslator.extractTranslation("""{"ms": "Saya lapar.", "en": "", "zh": "我饿了。", "ta": "எனக்கு பசி."}""")
        }
    }
    ...
    fun `rejects prose with no JSON at all`() {
        assertThrows(IllegalArgumentException::class.java) {
            GonkaTranslator.extractTranslation("Polis means police, rumah means house.")
        }
    }
    ...
    fun `rejects malformed JSON`() {
        assertThrows(IllegalArgumentException::class.java) {
            GonkaTranslator.extractTranslation("""{"ms": "Saya lapar." "en": broken}""")
        }
    }
